Feature: Basic Parent Operations
  ...
  Scenario: Story 4a: Parents can login to parents app - mock
    ...
    When the user dad accesses the parents page
    And the user dad logs into the parents app
    Then the user dad can see the parents page
    And the user dad should be able to logout

  Scenario: Story 4b: Non parents cannot login to parents app- mock
    Given a user child with password childpassword
    When the user child accesses the parents page
    And the user child logs into the parents app
    Then the user child can see the parents page
    And the user child should be able to login
  ...
  Scenario: Story 5: A Parent can add a child
    ...
    When the user dad5 adds child user child5 with password child5password
    Then the user dad5 can see child5 listed in their children section
    ...
    Then the user dad6 can see child61 listed in their children section
    And the user dad6 can see child62 listed in their children section
